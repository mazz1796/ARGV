...
def translate(sent)
    sent = sent.downcase
    vowels = ['a', 'e', 'i', 'o', 'u']
    word = sent.split(//) #a word changed into array: ["b","a","n","a","n","a"]
  

    word.each_with_index do |x, index|  
        if vowels.include? (x)  #vowels arrayで宣言した内容が含まれていれば。ちなみに()もあってもなくてもいい。
            return sent + "ay"  #このreturnもあってもなくてもいい。なんで２つめのインデックスにいかないかな？  
        else
...
result = [] #このarrayはあってもなくてもいい。
a.each do |x|
    result << translate(x)
end
# puts b = a.each { |i| translate(i)}
p result.join(' ')
#p ARGV
